...
//----------for open.qq.com
  if( chunk.toString().indexOf('<policy-file-request/>') != -1 ){
    socket._socket.write("<?xml version='1.0' ?><!DOCTYPE cross-domain-policy SYSTEM 'http://www.macromedia.com/xml/dtds/cross-domain-policy.dtd'><cross-domain-policy><allow-access-from domain='*' to-ports='*' /></cross-domain-policy>\0");
    return true;
  }

  if( chunk.toString().indexOf('tgw_l7_forward') != -1 ){
    return true;
  }

  //--check not utf8 encoding string
  var is_tgw_str = true;
  var TGW_BUFFER = new Buffer('tgw_l7_forward', 'utf8');
  if(chunk.length >= TGW_BUFFER.length*2 && chunk[0] == TGW_BUFFER[0]) {
    var len = Math.min(TGW_BUFFER.length*2, chunk.length);
    var j = 0;
    for (var i = 0; i < len; i++) {
      if(chunk[i] != 0){
        if(TGW_BUFFER[j] != chunk[i]){
          is_tgw_str = false;
          break;
        }
        j++;
